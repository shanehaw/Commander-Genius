...
#if SDL_VERSION_ATLEAST(2, 0, 0)
void TouchButton::clearFingers() { mFingerSet.clear(); }

void TouchButton::insertFingerId(const Sint64 fid) { mFingerSet.insert(fid); }
...
bool VirtualKeenControl::init() {
#if SDL_VERSION_ATLEAST(2, 0, 0)
  // Check if the virtual game pad image exist. If not download the picture
  // package
  ...
  {
    // Directional pad
    if (!mDPad.loadEmdbeddedPicture(gDPadPng, sizeof(gDPadPng)))
      return false;

    if (!mConfirmButton.loadEmdbeddedPicture(gButtonConfirmPng,
                                             sizeof(gButtonConfirmPng)))
      return false;
    mConfirmButton.invisible = true;

    if (!mStartButton.loadEmdbeddedPicture(gButtonStartPng,
                                           sizeof(gButtonStartPng)))
      return false;
    mStartButton.invisible = true;

    if (!mJumpButton.loadEmdbeddedPicture(gButton1Png, sizeof(gButton1Png)))
      return false;
    mJumpButton.invisible = true;

    if (!mPogoButton.loadEmdbeddedPicture(gButton2Png, sizeof(gButton2Png)))
      return false;
    mPogoButton.invisible = true;

    if (!mShootButton.loadEmdbeddedPicture(gButton3Png, sizeof(gButton3Png)))
      return false;
    mShootButton.invisible = true;

    if (!mStatusButton.loadEmdbeddedPicture(gButton4Png, sizeof(gButton4Png)))
      return false;
    mStatusButton.invisible = true;

    if (!mMenuButton.loadEmdbeddedPicture(gButtonMenuPng,
                                          sizeof(gButtonMenuPng)))
      return false;
...
void VirtualKeenControl::processConfig() { assert(0); }

void VirtualKeenControl::renderConfig() { assert(0); }
...
void VirtualKeenControl::render(GsWeakSurface &) {
  ...
  auto addTexture = [&vidDrv](TouchButton &button) -> void {
    if (!button.invisible && button.w > 0.0f && button.h > 0.0f) {
      if (button.isDown)
        vidDrv.addTextureRefToVirtPadRender(button.mHighlightTexture,
                                            button.Rect());
      else
  ...
  if (!mDPad.invisible && mDPad.w > 0.0f && mDPad.h > 0.0f) {
...
bool VirtualKeenControl::handleDPad(const GsVec2D<float> &Pos,
  ...
  auto fingerEv = [](const bool fingerDown, const SDL_Scancode scan,
...
bool VirtualKeenControl::mouseFingerState(
  ...
  auto bindButtonCommand = [&](TouchButton &button, std::string name,
    ...
    if (button.isInsideExpanded(Pos, 0.02f) && down && !button.isDown) {
      ...
      if (!isFinger || button.hasFinger(fingerID)) {
        button.isDown = false;
        stateChanged = true;
      }
    }
...
*/
  ...
  ok |= bindButtonCommand(mConfirmButton, "mConfirmButton", IC_JUMP);
  ok |= bindButtonCommand(mStartButton, "mStartButton", IC_JUMP);
  ok |= bindButtonCommand(mStatusButton, "mStatusButton", IC_STATUS);
  ok |= bindButtonCommand(mMenuButton, "mMenuButton", IC_BACK);
  ok |= bindButtonCommand(mShootButton, "mShootButton", IC_FIRE);
